...
g <- ggplot(activitySummaryByIntervalForImputedData, aes(interval, mean, colour = as.factor(weekdayFactor)))
g + geom_line(aes(group = weekdayFactor)) + labs(x = "Interval") + 
        labs(y = "Average number of steps taken across all days") +
        labs(title = "Time Series Plot for Imputed(Filled-In) Data")
```